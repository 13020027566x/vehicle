package com.finhub.framework.core.cache.guava;
    ...
    public ValueWrapper get(final Object key) {
        ...
        try {
            ...
            if (sw.getTime() > NUM_50) {
                log.info(
                    StrUtil.format("读取guava cache用时{}, key={}, cacheName={}", sw.getTime(), cacheKey, this.getName()));
            }
            return (value != null ? new SimpleValueWrapper(fromStoreValue(value)) : null);
        } catch (Exception e) {
            log.error(StrUtil.format("读取guava cache缓存发生异常, key={}, cacheName={}", cacheKey, this.getName()),
                e.getCause());
            return null;
        }
    }
