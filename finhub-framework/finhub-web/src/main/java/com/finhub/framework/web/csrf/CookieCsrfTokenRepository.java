...
public final class CookieCsrfTokenRepository implements CsrfTokenRepository, InitializingBean {
    ...
    public void saveToken(final CsrfTokenBean token, final HttpServletRequest request, final HttpServletResponse response) {
        String tokenValue = token == null ? StrConstants.S_EMPTY : token.getToken();
    ...
    public CsrfTokenBean loadTokenFromCache(final CsrfTokenBean token) {
        ...
        Object csrfTokenCacheVal = csrfTokenCache.get(csrfTokenCacheKey).get();
        if (Func.isNotEmpty(csrfTokenCacheVal)) {
            csrfTokenCache.evictIfPresent(csrfTokenCacheKey);
            return new CsrfTokenBean(this.headerName, this.parameterName, String.valueOf(csrfTokenCacheVal));
        }
        return null;
    }

    private String getTokenCacheKey(final CsrfTokenBean token) {
        return SecureUtil.md5(this.headerName + StrConstants.S_UNDERLINE + this.parameterName + StrConstants.S_UNDERLINE + (token == null ? StrConstants.S_EMPTY : token.getToken()));
    }
