      ...
      <Row>
        ...
        <Form ref="resourceForm" :model="resourceForm" :rules="ruleValidate" :label-width="80">
          ...
          <FormItem>
            ...
            <Button v-if="(resourceForm.id !== '' && options.modify) || (resourceForm.id === '' && options.add)"
      ...
      handleOk () {
        this.$refs.resourceForm.validate((valid) => {
          if (valid) {
            ...
            this.$store.dispatch(this.resourceForm.id === '' ? 'addResource' : 'modifyResource', {
              data: this.resourceForm
            }).then(() => {
              this.$Message.success((this.resourceForm.id === '' ? '资源新增' : '资源编辑') + '成功！')
              this.handleCancel()
              this.handlePageChange()
            }, (res) => {
