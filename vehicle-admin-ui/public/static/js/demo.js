...
function addTab (title) {
  ...
  $('#tt').tabs('add', {
    ...
    content: '<table id="dg' + i + '" class="easyui-datagrid" style="height:40%"' +
      'data-options="' +
      'url: \'http://localhost:58080/doc/listUrl\',' +
      'method: \'get\',' +
      'rownumbers:true,' +
      'singleSelect:true,' +
...
function listParams (row) {
  ...
  url = 'http://localhost:58080' + row['url']
  method = row['method']
  status = row['status']
  ...
  $.ajax({
    url: 'http://localhost:58080' + row['url'] + '?display=1',
    type: methodType,
    headers: headerMap,
    dataType: 'json',
      ...
      $('#tg').treegrid({
        ...
        loadFilter: function (res) {
          $('#pg').propertygrid({
            scrollbarSize: 0,
            columns: mycolumns,
            data: res.data.item.kv
          })

          $('#hg').propertygrid({
            scrollbarSize: 0,
            columns: headercolumns,
            data: res.data.item.header
          })

          return res.data.item.body
        },
...
$(function () {
  ...
  $.ajax({
    url: 'http://localhost:58080/doc/menu',
    type: 'get',
    dataType: 'json',
    success: function (res) {
      $('#menu').tree({
