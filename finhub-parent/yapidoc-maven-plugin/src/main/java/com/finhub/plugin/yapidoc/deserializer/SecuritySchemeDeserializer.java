...
public class SecuritySchemeDeserializer extends JsonDeserializer<SecurityScheme> {
    ...
    public SecurityScheme deserialize(JsonParser jp, DeserializationContext ctxt)
        ...
        if (inNode != null) {
            ...
            if ("http".equals(type)) {
                result.type(SecurityScheme.Type.HTTP).scheme(getFieldText("scheme", node))
                    .bearerFormat(getFieldText("bearerFormat", node));
            } else if ("apiKey".equals(type)) {
                result.type(SecurityScheme.Type.APIKEY).name(getFieldText("name", node))
                ...
                result.type(SecurityScheme.Type.OAUTH2)
