...
import com.finhub.plugin.yapidoc.ssh.SSHConfig;
import com.finhub.plugin.yapidoc.ssh.SSHCopy;
...
public class DocMojo extends AbstractMojo {
    ...
    private void execute(String url) {
        ...
        if (packageList != null) {
            ...
            controllerData = controllerData.stream().filter(javaClass -> {
                //log.info("Java class："+javaClass.getName()+"的包名："+javaClass.getPackageName());
                // 判断待处理的类的包是否在定义的包内
                for (String packagePrefix : packageList) {
                    if (javaClass.getPackageName().indexOf(packagePrefix) >= 0) {
                        return true;
                    }
                }
                return false;
        ...
        try {
            ...
            getLog().info("YAPI接口文档生成成功：" + filePath);

            if (StringUtils.isNotEmpty(yapiUrl)) {
                // 上传到Yapi服务器
                String result = uploadToYapiServer(filePath);
            }
    ...
    private String uploadToYapiServer(String filePath) throws Exception {
        ...
        String result = uploadToYapi.upload(openAPI, true);
