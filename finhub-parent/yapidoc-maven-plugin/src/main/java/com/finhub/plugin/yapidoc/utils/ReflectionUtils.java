...
import com.finhub.plugin.yapidoc.models.media.PrimitiveType;
...
public class ReflectionUtils {
    ...
    /**
     * Load Class by class name. If class not found in it's Class loader or one of
     * the parent class loaders - delegate to the Thread's ContextClassLoader
     *
    ...
    /**
     * Checks if the method methodToFind is the overridden method from the
            ...
            if (superClass != null && !(superClass.equals(Object.class))) {
                for (Method method : superClass.getMethods()) {
                    if (method.getName().equals(methodToFind.getName())
                        ...
                        methodToFind.getGenericParameterTypes())) {
                        return true;
                    }
                }
    ...
    /**
     * Returns overridden method from superclass if it exists. If method was not
     * found returns null.
     *
    ...
    /**
     * Searches the method methodToFind in given class cls. If the method is found
        ...
        for (Method method : cls.getMethods()) {
            if (method.getName().equals(methodToSearch)
                ...
                if (soughtForParameterType.length == srcParameterTypes.length
                    ...
                    if (hasIdenticalParameters(srcParameterTypes, soughtForParameterType, srcGenericParameterTypes,
    ...
    /**
     * Returns the list of declared fields from the class <code>cls</code> and its
     * superclasses excluding <code>Object</code> class. If the field from child
     * class hides the field from superclass, the field from superclass won't be
    ...
    public static boolean isSystemType(JavaType type) {
        // used while resolving container types to skip resolving system types; possibly
        // extend by checking classloader
