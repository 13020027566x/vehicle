...
public class TableInfo {
    ...
    public String getCamelName() {
        if (name.substring(0, 3).endsWith("_")) {
            return StrUtil.toCamelCase(name.substring(3));
        }
    ...
    public String getInsertValue() {
        StringBuilder sb = new StringBuilder();
        for (ColumnInfo column : columns) {
            if (StrUtil.equalsIgnoreCase("id", column.getName())) {
                continue;
            }
    ...
    public String getQueryParams() {
        StringBuilder sb = new StringBuilder();
        for (ColumnInfo column : columns) {
            if (StrUtil.equalsIgnoreCase("BigDecimal", column.getJavaType()) ||
                StrUtil.equalsIgnoreCase("String", column.getJavaType()) ||
                StrUtil.equalsIgnoreCase("byte[]", column.getJavaType()) ||
                StrUtil.equalsIgnoreCase("Date", column.getJavaType())) {
                sb.append(" #if(:1.").append(column.getCamelName()).append(" != null){")
